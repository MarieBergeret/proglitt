...
```{r}
df <- read.csv("data/PP_size_1.csv", header=TRUE, col.names=c("time"))
head(df);
```
...
La plupart des mesures se trouvent en dessous de 0,00005. Certaines fois le temps peut être jusqu'à 5 fois plus lent.
...
summary(df$time);
```
We can see that the mean estimation is `4.732e-5`, while the median gives `3.164e-5`. Other values are the minimum, the first quartile, the third quartile and the maximum number.

Les valeurs sont très faibles, de l'ordre de 10^-5.
...
Cet histogramme confirme ce que nous avons remarqué lors de la création du premier plot : la majorité des valeurs se situent entre 0 et 0,00005s. Plus la durée du ping pong augmente moins il y a de valeurs.
